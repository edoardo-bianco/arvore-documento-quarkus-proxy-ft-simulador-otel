...
| [0010 — Persistência de conformidade no CouchDB e checkpoints Flow no Redis](0010-conformidade-couchdb-checkpoint-redis-e-hitl-multipod.md) | Proposto | Propõe CouchDB para dados JSON e correlação de negócio, Redis/Valkey para checkpoints técnicos do Flow, `EventPublisher` e `_changes` -> `EventConsumer` para CloudEvents sem Kafka e Kubernetes Leases para múltiplos pods. | Alterar persistência, contexto/checkpoint do Flow, idempotência da revisão, bridge de eventos sem broker, containers, múltiplos pods ou IDs exibidos na API/página. |
