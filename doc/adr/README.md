...
| [0009 — PoC de conformidade com Quarkus Flow e HITL em memória](0009-poc-conformidade-quarkus-flow-hitl-em-memoria.md) | Aceito | Registra o baseline histórico da PoC com Flow, agente sequencial, Ollama e HITL volátil sem broker. A evolução durável implementada é governada pelo ADR-0010 ainda `Proposto`; por isso este ADR permanece `Aceito`, não `Substituído`. | Implementar ou atualizar workflow, agente, prompt, Ollama, FT, telemetria, canais `flow-in`/`flow-out`, CloudEvent, HITL, estado da PoC ou versões das extensões. |
| [0010 — Persistência documental por ambiente e checkpoints Flow no Redis](0010-conformidade-couchdb-checkpoint-redis-e-hitl-multipod.md) | Proposto | A cadeia reativa com porta documental, CouchDB local, Redis/Valkey, eventos referenciais, feed nativo e duas réplicas com Leases/failover foi implementada e comprovada localmente. Cosmos DB for NoSQL conserva contrato/SDK mockado; o gate real pré-PRD e a decisão humana de aceitação permanecem pendentes. | Alterar assincronia das portas/Flow/REST, persistência, adapter CouchDB/Cosmos, autenticação/RBAC Cosmos, contexto/checkpoint do Flow, idempotência da revisão, bridge de eventos sem broker, Compose Dev Services, containers, múltiplos pods ou IDs exibidos na API/página. |

## Ciclo de vida

- ADR novo começa como **Proposto** e exige checkpoint humano de arquitetura.
- GO de implementação não altera o status; somente decisão humana explícita de aceitação muda o
  ADR para **Aceito**.
