...
| [0009 — PoC de conformidade com Quarkus Flow e HITL em memória](0009-poc-conformidade-quarkus-flow-hitl-em-memoria.md) | Aceito | A PoC usa Flow, agente sequencial tipado, Ollama local com FT/fallback e canais internos sem broker; estado e revisão são voláteis, e logs/spans próprios não carregam conteúdo sensível. | Implementar ou atualizar workflow, agente, prompt, Ollama, FT, telemetria, canais `flow-in`/`flow-out`, CloudEvent, HITL, estado da PoC ou versões das extensões. |
| [0010 — Persistência documental por ambiente e checkpoints Flow no Redis](0010-conformidade-couchdb-checkpoint-redis-e-hitl-multipod.md) | Proposto | Propõe cadeia reativa com `Uni`, porta documental neutra, CouchDB automático em DES e Azure Cosmos DB for NoSQL em PRD autenticado por Entra ID/identidade gerenciada ou federada, contrato compartilhado, Redis/Valkey para checkpoints, `EventPublisher` e feed nativo -> `EventConsumer` sem Kafka; a validação multipod será posterior em Kubernetes local. | Alterar assincronia das portas/Flow/REST, persistência, adapter CouchDB/Cosmos, autenticação/RBAC Cosmos, contexto/checkpoint do Flow, idempotência da revisão, bridge de eventos sem broker, Compose Dev Services, containers, múltiplos pods ou IDs exibidos na API/página. |
