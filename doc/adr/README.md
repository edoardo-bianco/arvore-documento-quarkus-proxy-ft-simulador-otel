...
| [0010 — Persistência documental por ambiente e checkpoints Flow no Redis](0010-conformidade-couchdb-checkpoint-redis-e-hitl-multipod.md) | Proposto | Propõe porta documental neutra com CouchDB em DES e Azure Cosmos DB for NoSQL em PRD, contrato executável compartilhado, Redis/Valkey para checkpoints técnicos, `EventPublisher` e feed nativo -> `EventConsumer` para CloudEvents sem Kafka. | Alterar persistência, adapter CouchDB/Cosmos, autenticação Cosmos, contexto/checkpoint do Flow, idempotência da revisão, bridge de eventos sem broker, containers, múltiplos pods ou IDs exibidos na API/página. |
