...
public interface CriacaoDossieProdutoMtrClient {
    ...
    @Retry(maxRetries = 3, delay = 300, delayUnit = ChronoUnit.MILLIS,
            ...
            retryOn = {CriacaoDossieProdutoMtrException.Servidor.class,
                    ProcessingException.class, TimeoutException.class},
            abortOn = {CriacaoDossieProdutoMtrException.Negocio.class,
    ...
    @CircuitBreaker(requestVolumeThreshold = 10, failureRatio = 0.5,
            ...
            failOn = {CriacaoDossieProdutoMtrException.Servidor.class,
                    ProcessingException.class, TimeoutException.class},
            skipOn = {CriacaoDossieProdutoMtrException.Negocio.class,
                    CriacaoDossieProdutoMtrException.TecnicaCliente.class})
    Uni<CriacaoDossieProdutoMtrResponse> criar(CriacaoDossieProdutoMtrRequest requisicao);
    ...
    static RuntimeException toException(Response response) {
        ...
        CriacaoDossieProdutoMtrException.Erro erro = lerErro(response);
        if (status >= 500) {
            return new CriacaoDossieProdutoMtrException.Servidor(status, erro);
        }
        if (status == 400 || status == 404 || status == 409 || status == 422) {
            return new CriacaoDossieProdutoMtrException.Negocio(status, erro);
        }
        return new CriacaoDossieProdutoMtrException.TecnicaCliente(status, erro);
    }

    private static CriacaoDossieProdutoMtrException.Erro lerErro(Response response) {
        try {
            if (response.hasEntity()) {
                CriacaoDossieProdutoMtrException.Erro erro =
                        response.readEntity(CriacaoDossieProdutoMtrException.Erro.class);
                if (erro != null) {
                    return new CriacaoDossieProdutoMtrException.Erro(
        ...
        return new CriacaoDossieProdutoMtrException.Erro(
                ...
                "ARVDOCP0002",
                List.of(new CriacaoDossieProdutoMtrException.Mensagem(
