...
public class ProcessoParametrizadoMtrStubTestResource
        implements QuarkusTestResourceLifecycleManager {

    static final String CAMINHO_PROCESSO =
            "/simtr/parametrizacao/v2/patriarca/processo/identificador-negocial/";
    ...
    public Map<String, String> start() {
        try {
            ...
            throw new IllegalStateException("Nao foi possivel iniciar o stub MTR de processo", e);
        }

        executor = Executors.newFixedThreadPool(4);
        server.setExecutor(executor);
        server.createContext("/oidc/token", this::handleToken);
        server.createContext(CAMINHO_PROCESSO, this::handleProcesso);
        server.start();

        String baseUrl = "http://127.0.0.1:" + server.getAddress().getPort();
        return Map.of(
                "quarkus.rest-client.parametrizacao-processo.url", baseUrl + "/simtr",
                "quarkus.rest-client.parametrizacao-processo.read-timeout", "5000",
    ...
    private void handleProcesso(HttpExchange exchange) throws IOException {
